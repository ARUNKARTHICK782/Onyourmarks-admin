...
class AddTeacher extends StatefulWidget {
  ...
  final String mt;
  final String bg;
  const AddTeacher(this.isEditing,this.id, {Key? key, required this.name, required this.degree, required this.email, required this.status, required this.dob, required this.gender, required this.salary, required this.phNo, required this.curAdd, required this.perAdd, required this.mt, required this.bg}) : super(key: key);
...
class _AddTeacherState extends State<AddTeacher> {
  ...
  void initState() {
    ...
    if(widget.isEditing){
      ...
      _gender = (widget.gender == "Male")?Gender.Male:(widget.gender == "Female")?Gender.Female:Gender.Transgender;
      _status = (widget.gender == "Assigned")?Status.Assigned:Status.Unassigend;
    }
    else{
      _gender = Gender.Male;
                                            ...
                                            child:Padding(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  Padding(
                                                    padding: const EdgeInsets.only(left: 25),
                                                    child:(!widget.isEditing)?DropDownMultiSelect(
                                                      onChanged: (List<String> x) {
                                                        ...
                                                        tempList.forEach((element) {
                                                          if(selectedStandardIDs.contains(standardIDs.elementAt(standardNames.indexOf(element)))){
                                                            ...
                                                            selectedStandardIDs.add(standardIDs.elementAt(standardNames.indexOf(element)));
                                                          }
                                                        });
                                                    ...
                                                    ): Text("Cannot update standard here")
                                                  )
                                                ],
                                              ),
                                            ),
                                                ...
                                                child:Padding(
                                                  ...
                                                  child: Column(
                                                    ...
                                                    children: [
                                                      ...
                                                      Padding(
                                                        padding: const EdgeInsets.only(left: 25),
                                                        child:(allSubjectsName.isEmpty)?CircularProgressIndicator():(!widget.isEditing)? DropdownButton<String>(
                                                          value: tempSubject,
                                                          onChanged: (value){
                                                            setState(() {
                                                              tempSubject = value;
                                                              selectedSubject = allSubjectIDs.elementAt(allSubjectsName.indexOf(value!));
                                                            });
                                                          },
                                                          items : allSubjectsName.map<DropdownMenuItem<String>>((e){
                                                ...
                                                children: [
                                                  ...
                                                  Padding(
                                                    ...
                                                    child: DropdownButton<String>(
                                                      ...
                                                      items: <String>["Hindi","English","Bengali","Marathi","Telugu","Tamil","Gujarati","Urdu","Kannada","Odia","Malayalam","Punjabi","Assamese","Maithili","Manipuri","Sanskrit"].map<DropdownMenuItem<String>>((e){
                                                        return DropdownMenuItem(
                                                            value: e,
                                                            child: Text(e));
                                                      }).toList(),
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    (widget.isEditing)?ElevatedButton(onPressed: () async{
                                      TeacherModel teacher = TeacherModel.editTeacher(widget.id,nameCont.text, degreeCont.text, dateController.text,selectedGender, emailCont.text, phNoCont.text, curAddCont.text, perAddCont.text, selectedMotherTongue, selectedBG, salaryCont.text, selectedStatus);
                                      await updateTeacherDetails(teacher).then((v)=>{
                                       showDialog(context: context, builder: (BuildContext context){
                                         return ShowSuccessDialog("Teacher updated");
                                       })
                                      });
                                    }, child: Text("Save Changes"))
                                        :ElevatedButton(
                                        onPressed: () async{
                                          TeacherModel teacher = TeacherModel.addTeacher(nameCont.text, degreeCont.text, dateController.text,selectedGender, emailCont.text, phNoCont.text, curAddCont.text, perAddCont.text, selectedMotherTongue, selectedBG, salaryCont.text, selectedStatus,selectedSubject,selectedStandardIDs);
                                          await postTeacher(teacher).then((v)=>{
                                            showDialog(context: context, builder: (BuildContext context){
                                              return ShowSuccessDialog("Teacher added");
                                            })
                                          });
                                        },
