  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: FutureBuilder<List<StandardModel>>(
        ...
        builder: (BuildContext context,AsyncSnapshot<List<StandardModel>> snapshot){
                      ...
                      child: ExpansionTile(
                        title: Container(
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(snapshot.data?.elementAt(index).std_name ?? " ",style: TextStyle(fontSize: 25,fontWeight: FontWeight.w600),),
                              ],
                            ),
                          ),
                        ),
                        children: [
                          Padding(
                            ...
                            child: Column(
                              children: [
                                ListView.builder(itemBuilder: (BuildContext context,int index1){
                                  return Card(
                                    elevation: 2,
                                    child: SizedBox(
                                      height: 60,
                                      child: Padding(
                                        ...
                                        child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Text(snapshot.data?.elementAt(index).subjects?.elementAt(index1).subName ?? "",style: TextStyle(fontSize: 20),),
                                            Text(snapshot.data?.elementAt(index).subjects?.elementAt(index1).totalMarks ?? "",style: TextStyle(fontWeight: FontWeight.w600),),
                                          ],
                                        ),
                                      ),
                                    ),
                                ...
                                itemCount: snapshot.data?.elementAt(index).subjects?.length,
                                  shrinkWrap: true,
                                )
